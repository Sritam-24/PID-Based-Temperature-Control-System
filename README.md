...
- [Control Loop Flowchart](#-control-loop-flowchart)
...
```mermaid
flowchart TD
    A([Start]) --> B[Read temperature sensor]
    B --> C[Convert sensor voltage to °C]
    C --> D[Calculate Error = Setpoint − Temperature]
    D --> E[Compute PID Output<br/>Kp·Error + Ki·∫Error + Kd·dError/dt]
    E --> F[Clamp output to PWM range 30–255]
    F --> G[Write PWM signal to MOSFET gate D9]
    G --> H[MOSFET adjusts bulb brightness]
    H --> I[Bulb temperature changes]
    I --> J[Print temperature, error, PWM to Serial Monitor]
    J --> K{Wait one sample interval}
    K --> B
```

**Reading the flowchart:** every ~100 ms, the Arduino takes a fresh temperature reading, works out how far off it is from 32°C, runs that error through the PID formula, and turns the result into a PWM brightness level for the bulb. Because this repeats continuously, the system keeps correcting itself until the temperature settles at the setpoint.
